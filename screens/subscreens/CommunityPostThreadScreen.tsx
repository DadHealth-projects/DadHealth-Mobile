...
export default function CommunityPostThreadScreen() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`mobile-thread-${route.params.postId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'comments', filter: `post_id=eq.${route.params.postId}` }, () => void load()).subscribe();
    ...
    <SafeAreaView edges={['top']} className="flex-1 bg-dark">
      <ScrollView keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" automaticallyAdjustKeyboardInsets contentContainerClassName="px-lg pt-lg pb-xl gap-xl">
        ...
        {loading ? <View className="h-[240px] bg-white/5" /> : post ? <>
          ...
          {replyTo ? <View className="flex-row items-center justify-between"><Text className="font-body text-muted-text text-[12px]">Replying to {comments.find((comment) => comment.id === replyTo)?.author}</Text><Pressable onPress={() => setReplyTo(null)}><Text className="font-heading-bold text-lime text-[10px] uppercase">Cancel</Text></Pressable></View> : null}
          <TextInput value={draft} onChangeText={setDraft} multiline placeholder={replyTo ? 'Write a reply…' : 'Add a comment…'} placeholderTextColor={colors.tertiaryText} className="min-h-[90px] rounded-button border border-border bg-card p-md font-body text-white" />
