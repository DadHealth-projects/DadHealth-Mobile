...
type SharedEvent = {
  ...
  schedule_id: string;
  event_date: string;
  event_type: "handover" | "school" | "custody";
  notes: string | null;
};
...
const WEB_URL = (
  process.env.EXPO_PUBLIC_WEB_URL ?? "https://www.dadhealth.co.uk"
).replace(/\/$/, "");
...
export default function SharedCalendarScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (eventResult.error || milestoneResult?.error)
      ...
      setEvents((eventResult.data ?? []) as SharedEvent[]);
      setMilestones((milestoneResult?.data ?? []) as SharedMilestone[]);
    }
    setLoading(false);
  }, [isOffline, user?.id]);
    ...
    async (dateString: string) => {
      ...
      try {
        ...
        const next = current.includes(dateString)
          ? current.filter((item) => item !== dateString)
          : [...current, dateString].sort();
        if (schedule) {
          const { error: updateError } = await supabase
            .from("co_parenting_schedules")
            .update({ custody_dates: next })
            .eq("id", schedule.id)
            .eq("user_id", user?.id);
          ...
          const { error: insertError } = await supabase
            .from("co_parenting_schedules")
            .insert({ user_id: user?.id, custody_dates: next });
    ...
    try {
      ...
      const { error: addError } = await supabase
        ...
        .insert({
          ...
          event_date: localDate(eventDate),
  ...
  const removeEvent = useCallback(
    (eventId: string) =>
      Alert.alert(
        "Remove event?",
        "This event will be removed from the co-parenting calendar.",
        [
  ...
  const revoke = useCallback(
    () =>
      Alert.alert(
        "Revoke access?",
        "Your co-parent will immediately lose access to the shared calendar.",
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "Revoke",
  ...
  const markedDates = useMemo(
    () =>
      Object.fromEntries(
        (activeSchedule?.custody_dates ?? []).map((date) => [
          date,
          {
            selected: true,
  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-dark">
      ...
      >
        ...
        />
        <ScreenHero
          eyebrow="Co-parenting calendar"
          headline={"Parenting,\ntogether"}
          sub={
            readOnly
              ? "Your read-only view of custody days and shared events."
        ...
        {!user ? (
          ...
          <View className="h-[360px] bg-white/5 rounded-button" />
        ) : (
          <View className="gap-xl">
            {message ? (
              <Text
                accessibilityRole="alert"
                className="font-body text-tertiary-text text-[13px]"
              >
                {message}
              </Text>
            ) : null}
            <Text className="font-body text-muted-text text-[12px] leading-[18px]">
              {readOnly
                ? "Mood, journal, score and community activity are never shared."
                : "A co-parent receives read-only access. Your mood, sleep, journal, Dad Score and community activity stay private."}
            </Text>
            <View className="gap-sm">
              <Pressable
                onPress={() => setShowCustodyCalendar((visible) => !visible)}
                accessibilityRole="button"
                accessibilityState={{ expanded: showCustodyCalendar }}
                accessibilityLabel={
                  showCustodyCalendar
                    ? "Close custody calendar"
                    : "Open custody calendar"
                }
                className="min-h-[48px] flex-row items-center justify-between border-y border-border py-sm"
              >
                <View>
                  <Text className="font-heading-bold text-white text-[13px] uppercase">
                    Custody dates
                  </Text>
                  <Text className="font-body text-tertiary-text text-[11px] mt-xs">
                    {activeSchedule?.custody_dates?.length ?? 0} selected
                  </Text>
                </View>
                <Feather name="calendar" size={18} color={colors.lime} />
              </Pressable>
              {showCustodyCalendar ? (
                <Calendar
                  current={today()}
                  markedDates={markedDates}
                  onDayPress={(day) => void toggleCustodyDate(day.dateString)}
                  disableAllTouchEventsForDisabledDays
                  theme={{
                    calendarBackground: colors.card,
                    monthTextColor: colors.text,
                    dayTextColor: colors.text,
                    textDisabledColor: colors.tertiaryText,
                    arrowColor: colors.lime,
                    todayTextColor: colors.lime,
                    selectedDayTextColor: colors.dark,
                    textMonthFontFamily: "BarlowCondensed-Bold",
            ...
            {!readOnly ? (
              <>
                <View className="gap-md">
                  ...
                  {schedule?.co_parent_user_id ? (
                    <View className="gap-md border-y border-lime/25 py-md">
                      <Text className="font-body text-tertiary-text text-[13px]">
                        A co-parent is connected with read-only access.
                      </Text>
                      <Pressable onPress={revoke}>
                        <Text className="font-heading-bold text-red-300 text-[11px] uppercase">
                          Revoke access
                        </Text>
                      </Pressable>
                    </View>
                ...
                <View className="gap-md">
                  ...
                  <Pressable
                    onPress={() => setShowEventCalendar((visible) => !visible)}
                    accessibilityRole="button"
                    accessibilityState={{ expanded: showEventCalendar }}
                    accessibilityLabel={
                      showEventCalendar
                        ? "Close event calendar"
                        : "Open event calendar"
                    }
                    className="min-h-[48px] flex-row items-center justify-between border-y border-border py-sm"
                  >
                  ...
                  {showEventCalendar ? (
                    <Calendar
                      ...
                      onDayPress={(day) => {
                        setEventDate(new Date(`${day.dateString}T12:00:00`));
                  ...
                  {eventMessage ? (
                    <Text
                      accessibilityRole="alert"
                      className="font-body text-tertiary-text text-[12px]"
                    >
                      {eventMessage}
                    </Text>
                  ) : null}
            ...
            <View className="gap-sm">
              ...
              {events.length === 0 ? (
                <Text className="font-body text-muted-text">
                  No events yet.
                </Text>
              ) : (
                events.map((event) => (
                  <View
                    key={event.id}
                    className="flex-row gap-md border-b border-border py-md"
                  >
                    <View className="bg-lime px-sm py-xs self-start">
                      <Text className="font-heading-bold text-dark text-[10px] uppercase">
...
function TypeOption({
  ...
  return (
    <Pressable
      onPress={onPress}
      className={`flex-1 min-h-[44px] items-center justify-center border-b ${selected ? "border-lime" : "border-border"}`}
    >
      <Text
        className={`font-heading-bold text-[11px] uppercase ${selected ? "text-lime" : "text-muted-text"}`}
      >
        {label}
      </Text>
    </Pressable>
