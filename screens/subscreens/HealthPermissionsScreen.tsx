...
const HEALTH_CONNECT_DATA_TYPES = [
  ['activity', 'Steps', 'Your daily step total.'],
  ['clock', 'Active minutes', 'Exercise time from each day.'],
  ['heart', 'Resting heart rate', 'Your daily resting heart-rate estimate.'],
  ['moon', 'Sleep', 'Time Health Connect records as asleep.'],
] as const;

export default function HealthPermissionsScreen() {
  const navigation = useNavigation<NavigationProp<AppStackParamList>>();
  const { user } = useAuth();
  const appleHealth = useAppleHealth(user?.id);
  const healthConnect = useHealthConnect(user?.id);
  ...
  const dataTypes = isAndroid ? HEALTH_CONNECT_DATA_TYPES : APPLE_DATA_TYPES;
  const connected = Boolean(
    health.integration
      && (health.authorization === 'ready' || (isAndroid && health.authorization === 'partial')),
  );
  ...
  const connect = useCallback(async () => {
    ...
    if (result) setMessage(providerCopy.connectedMessage);
  }, [health.connect, providerCopy.connectedMessage]);

  const sync = useCallback(async () => {
    setMessage(null);
    const result = await health.sync();
    if (result) setMessage(providerCopy.syncedMessage);
  }, [health.sync, providerCopy.syncedMessage]);
  ...
  }, [healthConnect.openSettings, isAndroid]);
  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 bg-dark">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="px-lg pt-lg pb-[120px] gap-xl">
        ...
        <ScreenHero eyebrow="Settings" headline={'Health\npermissions'} sub={providerCopy.hero} />

        <View className="border-t border-border">
          {dataTypes.map(([icon, title, description]) => (
          ...
          {!user ? (
            ...
            <>
              <View className="flex-row items-center justify-between gap-md">
                <View className="flex-1">
                  <Text className="font-heading-bold text-lime text-[12px] uppercase">{providerCopy.connected}</Text>
                  <Text className="font-body text-muted-text text-[11px] leading-[17px] mt-xs">{isAndroid ? 'You can change what Dad Health can access anytime in Health Connect settings.' : 'You can change what Dad Health can access anytime in Apple Health or iPhone Settings.'}</Text>
                  <Text className="font-body text-tertiary-text text-[11px] mt-xs">{formatLastSync(health.integration?.last_sync_at ?? null)}</Text>
                </View>
            ...
            <>
              ...
              {isAndroid && health.capability === 'available' ? (
                <Pressable onPress={manageAccess} accessibilityRole="button" className="min-h-[44px] self-start justify-center border-b border-lime active:opacity-70">
                  <Text className="font-heading-bold text-lime text-[11px] uppercase">Manage Health Connect access</Text>
                </Pressable>
              ) : null}
            </>
          )}
        ...
        <ScreenErrorNotice message={health.error} />
