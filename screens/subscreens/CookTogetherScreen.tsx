...
export default function CookTogetherScreen() {
  ...
  const [openFilter, setOpenFilter] = useState<"difficulty" | "time" | null>(
    null,
  );
  ...
  const chooseTime = (value: "all" | number) => {
    recipeData.setFilters((current) => ({ ...current, maxMins: value }));
    setOpenFilter(null);
  };
  const difficultyLabel = DIFFICULTIES.find(
    (item) => item.value === recipeData.filters.difficulty,
  )?.label;
  const timeLabel = TIMES.find(
    (item) => item.value === recipeData.filters.maxMins,
  )?.label;
    ...
    <SafeAreaView edges={["top"]} className="flex-1 bg-dark">
      ...
      >
        ...
        <View className="flex-row items-center justify-between border-y border-border py-md">
          ...
          <Text className="font-heading-bold text-lime text-[22px]">
        ...
        <View className="flex-row gap-lg">
          <Filter
            label="Difficulty"
            value={difficultyLabel ?? "All difficulty"}
          ...
          <Filter
            label="Time"
            value={timeLabel ?? "Any time"}
            open={openFilter === "time"}
            onPress={() =>
              setOpenFilter((value) => (value === "time" ? null : "time"))
            }
          />
        </View>
        {openFilter === "difficulty" ? (
          <Options
            items={DIFFICULTIES}
            selected={recipeData.filters.difficulty}
        ...
        {openFilter === "time" ? (
          <Options
            items={TIMES}
            selected={recipeData.filters.maxMins}
            onSelect={chooseTime}
          />
        ) : null}
        ...
        {recipeData.loading ? (
          <View className="gap-sm">
            {[0, 1, 2].map((item) => (
              <View
                key={item}
                className="h-[220px] rounded-button bg-white/5"
              />
            ))}
          </View>
          ...
          <View className="gap-md">
            {recipeData.recipes.map((recipe) => {
              const saved = recipeData.savedIds.has(recipe.id);
              const completed = recipeData.completedIds.has(recipe.id);
              return (
                <View
                  key={recipe.id}
                  className="overflow-hidden rounded-button border border-border bg-card"
                >
                  {recipe.image_url ? (
                    <Image
                    ...
                    <View className="h-[110px] items-center justify-center bg-lime/5">
                      <Feather name="coffee" size={26} color={colors.lime} />
                    </View>
                  )}
                    ...
                    <View className="flex-row items-start gap-md">
                      <View className="flex-1">
                        ...
                        <Text className="font-body text-muted-text text-[13px] leading-[19px] mt-xs">
                          {recipe.description ??
                            "A simple recipe to cook with your kids."}
                        </Text>
                      </View>
                      <Pressable
                        onPress={() =>
                      ...
                      >
                        <Feather
                          name="bookmark"
                          size={21}
                          color={saved ? colors.lime : colors.tertiaryText}
                        />
                      </Pressable>
                    </View>
      ...
      >
        <SafeAreaView edges={["top", "bottom"]} className="flex-1 bg-dark">
          <ScrollView contentContainerClassName="px-lg py-lg gap-xl">
            ...
            <View>
              <Text className="font-heading-bold text-white text-[30px] uppercase">
                {active?.title}
              </Text>
            ...
            <View>
              <Text className="font-heading-bold text-lime text-[11px] uppercase mb-md">
                Ingredients
              </Text>
              ...
              {active?.steps.length ? (
                active.steps.map((step, index) => (
                  <View
                    ...
                    className="border-l-2 border-l-lime pl-md py-xs"
                  >
                    <Text className="font-heading-bold text-lime text-[11px] uppercase">
                      Step {index + 1}
                    </Text>
                    {step.title ? (
                      <Text className="font-heading-bold text-white text-[16px] uppercase mt-xs">
                        {step.title}
                      </Text>
                    ) : null}
                    ...
                    {step.kid_instruction ? (
                      <View className="bg-lime/10 border border-lime/20 p-md mt-md">
                        <Text className="font-body text-white text-[13px]">
                          <Text className="font-heading-bold text-lime uppercase">
                            Kid job:{" "}
                          </Text>
                          {step.kid_instruction}
                        </Text>
...
function Filter({
  ...
  return (
    <Pressable
      onPress={onPress}
      className="flex-1 border-b border-border pb-sm"
    >
      <Text className="font-heading-bold text-lime text-[9px] uppercase">
        {label}
      </Text>
      <View className="flex-row items-center justify-between mt-xs">
        <Text className="font-body text-white text-[13px]">{value}</Text>
        <Feather
    ...
    <View className="border-y border-border">
      {items.map((item) => (
        <Pressable
          ...
          className="min-h-[46px] flex-row items-center justify-between border-b border-white/5"
        >
          <Text className="font-body text-white text-[13px]">{item.label}</Text>
          {item.value === selected ? (
