...
export default function ActiveWorkoutScreen() {
  ...
  useLayoutEffect(() => {
    if (library.loading) reportScreenContentReady(false);
  }, [library.loading, reportScreenContentReady]);
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: colors.dark }}>
      ...
      >
        ...
        <View
          key={library.loading ? 'workout-skeleton' : 'workout-content'}
          onLayout={library.loading ? undefined : () => reportScreenContentReady(true)}
        >
          {library.loading ? (
