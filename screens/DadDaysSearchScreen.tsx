        ...
        {!user ? <LimeButton label="Log in to search" onPress={() => navigation.navigate('Login')} /> : (
          <View className="gap-xl">
            <View className="gap-md">
              ...
              <View className="flex-row items-center gap-sm"><Feather name={coords ? 'check-circle' : 'map-pin'} size={15} color={coords ? colors.lime : colors.tertiaryText} /><Text className="font-body text-white/45 text-[12px]">{locationLabel}</Text></View>
            </View>
            <View className="gap-md">
            ...
            {limitReached ? <View className="gap-md border-y border-border py-lg"><Text className="font-heading-bold text-white text-[17px] uppercase">3 free searches used</Text><Text className="font-body text-white/45 text-[13px]">Your allowance resets on the first of next month.</Text><LimeButton label="View Dad Health Pro" onPress={() => navigation.navigate('Tabs', { screen: 'Home' })} /></View> : <><LimeButton label="Search for Dad Days" onPress={() => void search()} loading={searching} />{!isPro ? <Text className="font-body text-white/40 text-[12px] text-center">{remaining} of {FREE_LIMIT} free searches remaining</Text> : null}</>}
          </View>
        )}
...
function DropdownTrigger({ icon, label, value, divided = false, open, onPress }: { icon: keyof typeof Feather.glyphMap; label: string; value: string; divided?: boolean; open: boolean; onPress: () => void }) {
  return <Pressable onPress={onPress} accessibilityRole="button" accessibilityState={{ expanded: open }} className={`flex-1 min-w-0 px-sm py-md active:opacity-75 ${divided ? 'border-l border-border' : ''}`}><View className="flex-row items-center gap-xs"><Feather name={icon} size={14} color={colors.lime} /><Text className="font-heading-bold text-white/40 text-[9px] uppercase">{label}</Text></View><View className="flex-row items-center gap-xs mt-xs"><Text numberOfLines={1} adjustsFontSizeToFit className="font-heading-bold text-white text-[12px] uppercase flex-1">{value}</Text><Feather name={open ? 'chevron-up' : 'chevron-down'} size={15} color={colors.lime} /></View></Pressable>;
}
